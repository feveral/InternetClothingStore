...
module.exports = class CheckoutManager{
	...
	CalculateTotal(data){
		...
		for(var dataIndex in data){
			totalClothNumber = totalClothNumber + data[dataIndex]['Quantity'];
			if(data[dataIndex]['PercentOff'])
				totalPrice = totalPrice + data[dataIndex]['Quantity']*data[dataIndex]['Price']*(1-data[dataIndex]['PercentOff']/100);
			else
				totalPrice = totalPrice + data[dataIndex]['Quantity']*data[dataIndex]['Price'];
		}
